...
export function Chat() {
  ...
  async function handleSend(message: string, files?: File[]) {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'assistant', content: "**API** stands for *Application Programming Interface*.\n\nIt’s a way for two programs to talk to each other. APIs define **how** requests and responses should look so different systems can work together.\n\nHere’s a simple example using JavaScript:\n\n```javascript\n// Example: Fetching data from an API\nasync function getUser() {\n\tconst response = await fetch('https://api.example.com/user/123');\n\tconst data = await response.json();\n\tconsole.log(data);\n}\n\ngetUser();\n```\n\n### Key Points\n- APIs let you **request** and **send** data between apps.\n- They often use **HTTP** and **JSON**.\n- Examples: OpenAI API, GitHub API, Weather API.\n"
      },
      ]);
    } finally {
      setIsLoading(false);
